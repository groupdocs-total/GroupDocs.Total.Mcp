...
public class AnnotationAddReplyToolTests
{
    ...
    public AnnotationAddReplyToolTests()
    {
        _output = new OutputHelper(_storage.Object, Microsoft.Extensions.Options.Options.Create(new McpConfig()));
    }

    [Fact]
    public async Task AnnotationAddReply_WhenResolverThrows_PropagatesException()
    {
        _resolver
        ...
        await Assert.ThrowsAsync<FileNotFoundException>(() =>
            GroupDocs.Total.Mcp.Tools.Annotation.AnnotationAddReplyTool.AnnotationAddReply(
                _resolver.Object,
                _storage.Object,
                _licenseManager.Object,
                _output,
                new FileInput { FilePath = "missing.pdf" },
                annotationId: 1, comment: "x"));
    }

    [Fact]
    public async Task AnnotationAddReply_WhenResolverThrows_DoesNotWriteToStorage()
    {
        _resolver
            .Setup(r => r.ResolveAsync(It.IsAny<FileInput>(), It.IsAny<CancellationToken>()))
            .ThrowsAsync(new FileNotFoundException("missing.pdf"));
    ...
    [Fact]
    public async Task AnnotationAddReply_SetsLicense_BeforeResolving()
    {
