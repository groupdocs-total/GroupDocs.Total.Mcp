...
Thanks for your interest in improving the GroupDocs.Total MCP server.
...
```bash
dotnet restore
dotnet build src/GroupDocs.Total.Mcp.sln -c Release
dotnet test  src/GroupDocs.Total.Mcp.sln -c Release
...
dotnet run --project src/GroupDocs.Total.Mcp
```

`.vscode/mcp.json` contains a `groupdocs-total-dev` entry that runs the server from
...
6. Integration tests against the *published* package live in the companion
   [GroupDocs.Total.Mcp.Tests](https://github.com/groupdocs-total/GroupDocs.Total.Mcp.Tests)
   repo — new tools or changed response shapes need a matching PR there.

## Reporting bugs / requesting features

Use the [issue templates](https://github.com/groupdocs-total/GroupDocs.Total.Mcp/issues/new/choose).
